(function ($) {
	...
	if ($('.skills-progress-bar').length) {
		...
		$('.skills-progress-bar').waypoint(function () {
			...
			$('.skillbar').each(function () {
				const $this = $(this);
				const percent = parseInt($this.attr('data-percent'), 10);
	...
	$("#contactForm").validator({ focus: false }).on("submit", function (event) {
        if (!event.isDefaultPrevented()) {
            ...
            submitForm("#contactForm", "../microvilla-html/form-process.php", contactFormSuccess);
        }
    });
	...
	$("#enquiryForm").validator({ focus: false }).on("submit", function (event) {
		if(!event.isDefaultPrevented()) {
			event.preventDefault();
			submitForm("#enquiryForm","../microvilla-html/booking-form.php",appointmentFormSuccess);
		}
	});
	...
	let lastScrollTop = 0;
		...
		window.addEventListener('scroll', function () {
			const img = document.querySelector('#sisf-m-image'); // target only <img>
			if (!img) return;
			...
			img.style.transition = "transform 0.1s linear"; // smooth rotation
	...
	if ($('#daterange').length) {
		flatpickr("#daterange", {
			mode: "range",
			dateFormat: "Y-m-d", 
			defaultDate: ["2025-09-29", "2025-09-30"],
		});
	}
})(jQuery);